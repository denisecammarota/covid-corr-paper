...
source('fct/sir_mp_fix_optim_matform.R')
# Actual function that interests us ------------------------------
int_sir_mp_fix_optim <- function(pars, time, tseries_2, state, matA, n_days, n_provs, gamma){
  # calculating the ode solving values
  out <- deSolve::ode(y=state,func = sir_mp_fix_optim_matform,parms=pars,
